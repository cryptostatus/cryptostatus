class ReportQuery
  ...
  def self.report
    @report ||= Currency.select(:name, 'MAX(yesterday_currencies.price) - MAX(currencies.price) as max_diff', 'MIN(yesterday_currencies.price) - MIN(currencies.price) as min_diff')
